...
As an alternative to running BentoPDF locally or in a Docker container, you can very easily host it as a set of static web pages. Here are a few examples:

## Quick Deploy (Recommended)

The easiest way to deploy BentoPDF as a webpage is to use one of these one-click deploy buttons:
...
- Deploy your site within minutes
- Provide automatic deployments when you push updates
- Offer free hosting with generous limits
...
BentoPDF includes pre-configured deployment files for seamless deployment:

### Vercel (`vercel.json`)

- Configures Vite as the build framework
- Sets proper output directory (`dist`)
- Includes security headers (CSP, XSS protection, etc.)
...
- Implements SPA redirects for proper routing
- Includes security headers
- Supports gzip and brotli compression
- Also includes `public/_redirects` for alternative redirect configuration

These configuration files are already included in the repository, so no manual setup is required when using the one-click deploy buttons.
